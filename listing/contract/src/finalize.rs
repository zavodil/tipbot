...
#[near_bindgen]
impl ListingAuction {
    pub fn finalize(&mut self, auction_id: AuctionId) {
        let mut auction: Auction = self.unwrap_auction(&auction_id);
        self.assert_auction_end_date_already_passed(&auction);

        assert!(auction.winner_id.is_none(), "ERR_ALREADY_FINALIZED");
        assert!(auction.tokens.len() > 0, "ERR_NO_TOKENS");
        ...
        auction.winner_id = Some(winner_account_id);
    ...
    pub fn withdraw_rewards(&mut self, auction_id: AuctionId, token_id: TokenId) -> Promise {
        ...
        if let Some(winner_id) = auction.winner_id.clone() {
            ...
            let balance_to_withdraw = auction.rewards.get(&key).expect("ERR_NOTHING_TO_CLAIM");
    ...
    pub fn withdraw_bids(&mut self, auction_id: AuctionId, token_id: TokenId) -> Promise {
        ...
        if let Some(winner_id) = auction.winner_id.clone() {
            ...
            let key = get_auction_deposit(account_id.clone(), token_id.clone());
            let balance_to_withdraw = auction.deposits.get(&key).expect("ERR_NOTHING_TO_CLAIM");
    ...
    pub fn claim_reward(&mut self, auction_id: AuctionId, token_id: TokenId) -> Promise {
        let mut auction: Auction = self.unwrap_auction(&auction_id);
        let account_id = env::predecessor_account_id();
       ...
       if let Some(winner_id) = auction.winner_id.clone() {
            ...
            if winner_id == token_id {
               ...
               env::panic_str("ERR_THIS_TOKEN_IS_NOT_A_WINNER")
